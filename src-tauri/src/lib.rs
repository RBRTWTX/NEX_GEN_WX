...
#[tauri::command]
async fn fetch_tropical_arrival_time_catalog(
    mode: String,
    force: bool,
) -> Result<Value, error::StudioError> {
    weather_engine::providers::tropical_arrival_time_catalog(&mode, force).await
}

#[tauri::command]
async fn fetch_tropical_storm_surge_catalog(
    product: String,
    force: bool,
) -> Result<Value, error::StudioError> {
    weather_engine::providers::tropical_storm_surge_catalog(&product, force).await
}

#[tauri::command]
async fn fetch_model_hrrr_cycle_catalog(
    date: String,
    cycle: u8,
    force: bool,
) -> Result<Value, error::StudioError> {
    weather_engine::providers::hrrr_cycle_catalog(&date, cycle, force).await
}

#[tauri::command]
async fn fetch_model_hrrr_field(
    date: String,
    cycle: u8,
    forecast_hour: u16,
    field: String,
    ...
    weather_engine::providers::hrrr_field(
        &date,
        cycle,
        forecast_hour,
        &field,
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            fetch_model_hrrr_cycle_catalog,
            fetch_model_hrrr_field
        ])
